					...
					for ( $i = 2; $i < count( $tag_components ); $i++ ) {
						...
						if ( count( $sub_components ) == 1 ) {
							...
							if ( $sub_components[0] == 'input type' ) {
								...
								if( $semantic_property && !empty($semantic_property) /*&& array_key_exists('mapping template', $field_args)*/ ) {

									$fakeTemplateField = new SFTemplateField();
									$fakeTemplateField->setSemanticProperty( $semantic_property );
									$possible_values = $fakeTemplateField->getPossibleValues();
...
END;
					...
					if ( $template_name === '' || $field_name == '<freetext>' ) {
						...
						if ( ! empty( $values ) && $possible_values == null ) {
							// Remove whitespaces, and un-escape characters
							$possible_values = array_map( 'trim', explode( $delimiter, $values ) );
							$possible_values = array_map( 'htmlspecialchars_decode', $possible_values );
						}
